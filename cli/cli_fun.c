...
{   
    ...
    memcpy(send_buff,"hd",2);
    memcpy(send_buff+2,S_path,strlen(S_path));
    memcpy(send_buff+strlen(S_path)+2+1,O_path,strlen(O_path));
...
//同步test文件夹
    ...
    int wd = inotify_add_watch(i_fd,dir_path,IN_CREATE|IN_DELETE|IN_MOVE|IN_MODIFY|IN_DELETE_SELF);
